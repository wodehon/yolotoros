...
class yolo_detect():
    def __init__(self):
        self.yolo = detect_test.detectapi(weights = '/home/duan/Code/my_yolo5/runs/train/exp23/weights/best.pt')
    ...
    def detectimg(self, img, img_depth):
        ...
        for i in result[0][1]:
            ...
            pointxyz = [world_x * depth_ * 0.001, world_y * depth_ * 0.001, depth_ * 0.001]
            point_ = cal(pointxyz)
            self.pos_msg.position.x = point_[1]
            self.pos_msg.position.y = point_[2]
            self.pos_msg.position.z = point_[3]
...
def cal(point):
    ...
    point_ = np.dot(point, rot) + np.array(trans)
