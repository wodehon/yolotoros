...
class yolo_detect():
    def __init__(self):
        ...
        # just
        self.color_sub = rospy.Subscriber('/camera/color/image_raw', Image, self.detectimg)
        ...
        self.scale = 0.001    # 真实相机为0.001，仿真为1，参数，和 z反比变化
        ...
        self.MAX_Z = 10000 #参数
        self.depth_min = 10000 # 初始值
        self.count = 0 # 校准的循环指数
    ...
    def pose_cb(self,cam_pose):
        self.trans[0] = cam_pose.pose.position.x
        self.trans[1] = cam_pose.pose.position.y
        self.trans[2] = cam_pose.pose.position.z
        
        self.quat[0] = cam_pose.pose.orientation.w
        self.quat[1] = cam_pose.pose.orientation.x
        self.quat[2] = cam_pose.pose.orientation.y
        self.quat[3] = cam_pose.pose.orientation.z

    def depth_cb(self,msg):
        # self.depth_img = msg
        self.frame_depth = self.bridge.imgmsg_to_cv2(msg, desired_encoding='16UC1')
    ...
    def detectimg(self, img):
        ...
        for i in result[0][1]:
            ...
            depth_ = self.frame_depth[int(center_y),int(center_x)]
            if depth_ == 0: # 无效深度(设置最大值)
                depth_ = self.MAX_Z
            # if id == "tower": # 如果是电塔，则以最远为目标点
            #     depth_ = self.MAX_Z
            
            # obj_pose.pose.pose.position.x = world_x * depth_ * self.scale
            # obj_pose.pose.pose.position.y = world_y * depth_ * self.scale
            # obj_pose.pose.pose.position.z =  depth_ * self.scale   #2D相机则显示,归一化后的结果,用户用时自行乘上深度z获取正确xy
            # # print(obj_pose)
            ...
            # 在每次yolo的过程中进行规划
            if name == "tower" and obj_pose.score > 0.7 and (not self.trigger):
                ...
                if not self.triggertest:
                    if center_x < x/2 and self.tuning_msg.data != -1:
                        ...
                        print("3")
                    self.count+=1
                if self.tuning_msg.data == 0 and self.count > 10: #校准的循环次数
                    print("___pub___")
                    self.triggertest = True
                    depth_ = self.MAX_Z
                    obj_pose.pose.pose.position.x = world_x * depth_ * self.scale
                    obj_pose.pose.pose.position.y = world_y * depth_ * self.scale
                    obj_pose.pose.pose.position.z =  depth_ * self.scale
                    pointxyz = [obj_pose.pose.pose.position.x, obj_pose.pose.pose.position.y, obj_pose.pose.pose.position.z]
                    print("depth_="+ str(depth_))
                    print("对准的重点")
                    print(point,[world_x,world_y],pointxyz)
                    point_ = cal(pointxyz, self.trans, self.quat)
                    if(abs(point_[0]) > 2.5 or abs(point_[1]) > 2.5 or point_[2] > 2.49 or point_[2] < 0):
                        point_ = trunc(point_, self.trans)
                    ...
                    # 1. 使用yolo检测结果
                    # self.pose_pub.publish(self.pose_msg)
                    print("x=" + str(self.pose_msg.pose.position.x)+",y="+str(self.pose_msg.pose.position.y)+",z="+ str(self.pose_msg.pose.position.z))
                    print("====================")
                    # 2. 直接发终点的规划
                    self.pose_msg.pose.position.x = -0.9407164
                    self.pose_msg.pose.position.y = 0.7008502
                    self.pose_msg.pose.position.z = 1.0891195
                    self.pose_pub.publish(self.pose_msg)
                    print("一阶段   x=-0.9407164" + ",y=0.7008502"+",z=1.0891195")
                    
            if self.triggertest and name == "insulator" and obj_pose.score > 0.7 and depth_ > 1000 and depth_ < 3000 and depth_ < self.depth_min: # 6m开始切换adjust,且以最近的那个为目标
                self.trigger = True
                self.tri_msg.data = self.trigger
                self.depth_min = depth_
                print("==================")
                print("===",depth_,depth_<5000,self.depth_min,self.trigger)
                ...
                print("-----------")
                print(point_)
                yaw = math.atan((point_[1]-self.trans[1])/(point_[0]-self.trans[0]))
                self.pose_msg.pose.orientation.w = yaw
                ...
                # else : # 往右
                #     pointxyz[0] = pointxyz[0] + sin_ * self.distance
                pointxyz[0] = pointxyz[0] - sin_ * max((depth_ -1),self.distance) # 实验往左
        ...
        if self.trigger:
            ...
            self.pose_msg.pose.position.y = 0.7008502
            self.pose_msg.pose.position.z = 1.0891195
            print("阶段二  x=-0.9407164" + ",y=0.7008502"+",z=1.0891195")
        ...
        self.img_pub.publish(self.bridge.cv2_to_imgmsg(image_detect, "bgr8"))
        print("------coss " + str(time3.to_sec() - time1.to_sec()) + " s-------")
...
def quattorot(quat_):
    # return tf.self.transformations.quaternion_matrix(quat_)
    q = quat_.copy()
    q /= np.linalg.norm(q)
    n = np.dot(q, q)
...
def cal(point, trans_, quat_):
    # get_tf = False
    # while not get_tf:
    # (self.trans,self.quat) = listener.lookupself.transform('/turtle2', '/turtle1', rospy.Time(0))
...
def trunc(point, trans_): # 需要调节的参数
    range_x = 2.5
    range_y = 2.5
    range_z = 2.5 - 0.1
...
if __name__ == '__main__':
    ...
    # listener = tf.self.transformListener()
    yolo_detect()
    rospy.spin()
